...
function generateWorkbook() {
	...
	invoiceWS
		.cell(4, 2, 4, 3, true)
		.string("PAYMENT DUE BY: March 1, 2016")
		.style({
			font: {
				bold: true,
			},
	...
	while (i <= 10) {
		...
		if (item !== undefined) {
			...
			invoiceWS
				.cell(curRow, 5)
				.formula(
					xl.getExcelCellRef(rowOffset + i, 2) +
						"*" +
						xl.getExcelCellRef(rowOffset + 1, 4)
				)
				.style(currencyStyle);
		}
		if (i % 2 === 0) {
	...
	invoiceWS.cell(22, 5).number(0.0).style(currencyStyle);
	...
	for (var i = 1; i <= 10; i++) {
		collapseSheet
			...
			.formula(
				"SUM(" +
					xl.getExcelCellRef(rowOffset + 1, i) +
					":" +
					xl.getExcelCellRef(rowOffset + 10, i) +
					")"
			);
	}
	...
	optionsSheet
		.cell(1, 2)
		.string("Option List")
		.comment("Values from some reference doc");
	optionsSheet.cell(1, 3).string("Numbers 1-10");
	optionsSheet
		.cell(1, 3)
		.comment(
			"Comment: Number defined by some other reference doc with comment visible by default",
			{
				marginLeft: "300pt",
				marginTop: "20pt",
				width: "400px",
	...
	optionsSheet.addDataValidation({
		...
		sqref: "B2:B10",
		formulas: ["option 1,option 2,option 3"],
	});
	...
	unicodeSheet
		.cell(7, 2)
		.string("Good [crossed fingers: light skin tone] luck");
	...
	unicodeSheet
		.cell(8, 2)
		.string(
			"Fist [right-facing fist: light skin tone][left-facing fist: dark skin tone] bump"
		);
	unicodeSheet
		.cell(8, 3)
		.string("Fist &#x1f91c;&#x1f3fb;&#x1f91b;&#x1f3ff; bump");
	...
	unicodeSheet
		.cell(9, 2)
		.string(
			"I am the [greek capital letter alpha] and the [greek capital letter omega]"
		);
	...
	imageSheet.addImage({
		image: fs.readFileSync(
			path.resolve(__dirname, "./sampleFiles/thumbsUp.jpg")
		),
		type: "picture",
		position: {
			type: "oneCellAnchor",
