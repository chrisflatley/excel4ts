...
export class Workbook {
	...
	constructor(opts = {}) {
		...
		const hasValidCustomLogger =
			hasCustomLogger &&
			typeof opts.logger.warn === "function" &&
			typeof opts.logger.error === "function";

		this.logger = hasValidCustomLogger
			? opts.logger
			: new SimpleLogger({
					logLevel: Number.isNaN(parseInt(opts.logLevel))
						? 0
						: parseInt(opts.logLevel),
			  });
