...
let addWorkbookXML = (promiseObj) => {
	// Required as stated in §12.2
	return new Promise((resolve, reject) => {
		let xml = xmlbuilder.create("workbook", {
			version: "1.0",
			encoding: "UTF-8",
			standalone: true,
			allowSurrogateChars: true,
		...
		if (promiseObj.wb.opts.workbookView) {
			...
			if (viewOpts.autoFilterDateGrouping) {
				workbookViewEle.att(
					...
					utils.boolToInt(viewOpts.autoFilterDateGrouping)
				);
			}
			...
			if (viewOpts.showHorizontalScroll) {
				workbookViewEle.att(
					...
					utils.boolToInt(viewOpts.showHorizontalScroll)
				);
			}
			if (viewOpts.showSheetTabs) {
				workbookViewEle.att(
					"showSheetTabs",
					utils.boolToInt(viewOpts.showSheetTabs)
				);
			}
			if (viewOpts.showVerticalScroll) {
				workbookViewEle.att(
					"showVerticalScroll",
					utils.boolToInt(viewOpts.showVerticalScroll)
				);
			}
		...
		promiseObj.wb.sheets.forEach((s, i) => {
			...
			if (s.printArea) {
				const name = s.name;
				const startCellRef = `$${utils.getExcelAlpha(s.printArea.startCol)}$${
					s.printArea.startRow
				}`;
				const endCellRef = `$${utils.getExcelAlpha(s.printArea.endCol)}$${
					s.printArea.endRow
				}`;
...
let addWorkbookRelsXML = (promiseObj) => {
	...
	return new Promise((resolve, reject) => {
		...
		promiseObj.xlsx
			.folder("xl")
			.folder("_rels")
			.file("workbook.xml.rels", xmlString);
		resolve(promiseObj);
	});
};
						...
						return new Promise((resolve) => {
							if (xml) {
								promiseObj.xlsx
									...
									.folder("worksheets")
									.folder("_rels")
									.file(`sheet${curSheet}.xml.rels`, xml);
							}
							resolve();
						});
					})
					...
					.then((xml) => {
						return new Promise((resolve) => {
							if (xml) {
								promiseObj.xlsx
									.folder("xl")
									.file(`comments${curSheet}.xml`, xml);
							}
							resolve();
						});
					})
						...
						return new Promise((resolve) => {
							if (xml) {
								promiseObj.xlsx
									.folder("xl")
									.folder("drawings")
									.file(`commentsVml${curSheet}.vml`, xml);
							}
							resolve();
						});
					})
...
let addStylesXML = (promiseObj) => {
	...
	return new Promise((resolve, reject) => {
		...
		let cellStyleXfsXML = xml.ele("cellStyleXfs").att("count", 1);
		...
		let cellStylesXML = xml.ele("cellStyles").att("count", 1);
		cellStylesXML
			.ele("cellStyle")
			.att("name", "Normal")
			.att("xfId", "0")
		...
		if (!promiseObj.wb.mediaCollection.isEmpty) {
			promiseObj.wb.sheets.forEach((ws) => {
				if (!ws.drawingCollection.isEmpty) {
					...
					let drawingsXML = xmlbuilder.create("xdr:wsDr", {
						version: "1.0",
						encoding: "UTF-8",
						standalone: true,
						allowSurrogateChars: true,
					...
					promiseObj.xlsx
						.folder("xl")
						.folder("drawings")
						.file("drawing" + ws.sheetId + ".xml", drawingsXMLStr);
					promiseObj.xlsx
						.folder("xl")
						.folder("drawings")
						.folder("_rels")
						.file("drawing" + ws.sheetId + ".xml.rels", drawingRelXMLStr);
				}
			});
		}
		resolve(promiseObj);
