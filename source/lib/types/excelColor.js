...
items.prototype.validate = function (val) {
  if (this[val.toLowerCase()] === undefined) {
    ...
    throw new TypeError(
      "Invalid value for ST_PresetColorVal; Value must be one of " +
        this.opts.join(", ")
    );
  } else {
    return true;
...
items.prototype.getColor = function (val) {
  ...
  if (typeof this[val.toLowerCase()] === "string") {
    ...
    throw new TypeError(
      "valid color options are html style hex codes, ARGB strings or these colors by name: %s",
      this.opts.join(", ")
    );
  }
};
