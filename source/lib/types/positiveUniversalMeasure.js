...
measure.prototype.validate = function (val) {
	...
	if (re.test(val) !== true) {
		throw new TypeError(
			"Invalid value for universal positive measure. Value must a positive Float immediately followed by unit of measure from list mm, cm, in, pt, pc, pi. i.e. 10.5cm"
		);
	} else {
		return true;
	}
