...
class Picture extends Drawing {
  ...
  constructor(opts) {
    ...
    this._name = this.image ? opts.name || uniqueId('image-') : opts.name || path.basename(this.imagePath);
    ...
    this._extension = this.image ? size.type : path.extname(this.imagePath).substr(1);
