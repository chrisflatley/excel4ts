...
let _addDimension = (promiseObj) => {
	...
	return new Promise((resolve, reject) => {
		let firstCell = "A1";
		let lastCell = `${utils.getExcelAlpha(promiseObj.ws.lastUsedCol)}${
	...
	return new Promise((resolve, reject) => {
		...
		let processRows = (theseRows) => {
			for (var r = 0; r < theseRows.length; r++) {
				...
				if (
					promiseObj.ws.opts.disableRowSpansOptimization !== true &&
					thisRow.spans
				) {
...
let _addHeaderFooter = (promiseObj) => {
	...
	return new Promise((resolve, reject) => {
		...
		if (addHeaderFooter === true) {
			...
			o.oddHeader !== null
				? hfEle.ele("oddHeader").text(o.oddHeader).up()
				: null;
			o.oddFooter !== null
				? hfEle.ele("oddFooter").text(o.oddFooter).up()
				: null;
			o.evenHeader !== null
				? hfEle.ele("evenHeader").text(o.evenHeader).up()
				: null;
			o.evenFooter !== null
				? hfEle.ele("evenFooter").text(o.evenFooter).up()
				: null;
			o.firstHeader !== null
				? hfEle.ele("firstHeader").text(o.firstHeader).up()
				: null;
			o.firstFooter !== null
				? hfEle.ele("firstFooter").text(o.firstFooter).up()
				: null;
			hfEle.up();
		}
...
export const relsXML = (ws) => {
	return new Promise((resolve, reject) => {
		...
		let relXML = xml.create("Relationships", {
			version: "1.0",
			encoding: "UTF-8",
			standalone: true,
			allowSurrogateChars: true,
...
export const commentsVmlXML = (ws) => {
	return new Promise((resolve, reject) => {
		...
		Object.keys(ws.comments).forEach((ref) => {
			...
			shape.att(
				"style",
				`position:${position};margin-left:${marginLeft};margin-top:${marginTop};width:${width};height:${height};z-index:${zIndex};visibility:${visibility}`
			);
