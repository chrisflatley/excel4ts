...
test("Cell returns correct number of cell references", (t) => {
	...
	t.ok(
		cellAccessor.excelRefs.length === 10,
		"cellAccessor returns correct number of cellRefs"
	);
});
...
test("Add String to cell", (t) => {
	...
	t.ok(
		wb.sharedStrings[thisCell.v] === "my test string",
		"Cell sharedString value is correct"
	);
});
...
test("Add Boolean to cell", (t) => {
	...
	t.ok(
		thisCell.v === "true" || thisCell.v === "false",
		"Cell value value is correct"
	);
});
...
test("Add Comment to cell", (t) => {
	...
	ws.generateCommentsXML().then((XML) => {
		...
		t.ok(
			testComment.textContent === "My test comment",
			"Verify comment text is correct"
		);
		t.end();
	});
});
