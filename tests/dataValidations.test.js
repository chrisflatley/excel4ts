...
test("DataValidation Tests", (t) => {
	...
	t.ok(
		val1.formula1 === 0 && val1.formula2 === undefined,
		"formula's of first validation correctly set"
	);
	t.ok(
		val2.formula1 === "value1,value2" && val2.formula2 === undefined,
		"formula's of 2nd validation correctly set"
	);
	t.ok(
		val3.formula1 === "value1,value2" && val3.formula2 === undefined,
		"formula's of 3rd validation correctly set"
	);
	t.ok(
		val5.formula1 === 0 && val5.formula2 === 10,
		"formula's of 4th validation correctly set"
	);
	try {
		...
		t.ok(
			val6 instanceof DataValidation === false,
	...
	ws.generateXML().then((XML) => {
		...
		t.equals(
			dataValidations[1].getAttribute("showDropDown"),
			"",
			"showDropDown correclty not set when showDropDown is set to true"
		);
		t.equals(
			dataValidations[2].getAttribute("showDropDown"),
			"",
			"showDropDown correclty not set when showDropDown is not specified"
		);
		t.equals(
			dataValidations[3].getAttribute("showDropDown"),
			"1",
			"showDropDown correclty set to 1 when showDropDown is set to false"
		);
		t.end();
	});
});
